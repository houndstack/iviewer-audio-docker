...
//             console.log("Enter updateAnnotation", item.ann.id, item, query);
    ...
    async stopRecord() {
        ...
        console.log("Stopped Recording", this.recorderQueue)
        ...
        this.recordAnnotations(text)
            ...
            // for(let i = 0;i<this.recorderQueue.length-1; i++){
            //     if(text[index]['timestamp'][0]>=this.recorderQueue[i]['timestamp']&&text[index]['timestamp'][0]<=this.recorderQueue[i+1]['timestamp']){
            //         this.recorderQueue[i].annotation = text[index]['text']
            //         annotationList.push(this.recorderQueue[i])
            //         index++;
            //     }
            // }
    ...
    async recordAnnotations(text) {
        ...
        console.log(this.recorderQueue)
        let i = 0
        this.recorderQueue.push({'timestamp': 99999999999999})
        while(i<this.recorderQueue.length-1){
    ...
    recordInterval() {
        ...
        let timestamp = Date.now()
        console.log(this.recorderQueue, this.recorderQueue[this.recorderQueue.length - 1], newQuery)
        if(JSON.stringify(this.recorderQueue[this.recorderQueue.length - 1])!==JSON.stringify(newQuery)){
            newQuery.timestamp = timestamp
            this.recorderQueue.push(newQuery)
